...
for (k in 1:nrow(conditions)) {
  ...
  for (i in c(TRUE, FALSE)) {
    anticlusters <- anticlustPackage:::exact_anticlustering(as.matrix(dist(features)),
                                         p_anticlusters, preclustering = i, NULL)
    anti_list[[i + 1]] <- anticlusters
    # Allow for some numeric imprecision of ILP solver:
    obj_values[i + 1]  <- round(anticlustPackage:::diversity_objective_(anticlusters, features), 10)
  }
